...
export default {
  ...
  props: {
    ...
    partialBatch: { type: Boolean, default: false },
  ...
  methods: {
    initializeData(items, param = null) {
      if (items && items.length > 0) {
        for (let i in items) {
          if (param) {
            if (param.hasOwnProperty("selected")) {
              items[i].selected = param.selected || false;
              items[i].partial = false;
            }
            if (param.hasOwnProperty("opened")) {
              items[i].opened = param.opened || false;
            }
          }

          let dataItem = this.initializeDataItem(items[i]);
          items[i] = dataItem;
          this.initializeData(items[i][this.childrenFieldName], param);
        }
      } else {
        this.$forceUpdate();
      }
    ...
    initializeDataItem(item) {
      function Model(item, textFieldName, valueFieldName, childrenFieldName, collapse) {
        ...
        this.partial = item.partial || false;
    ...
    handleBatchSelectItems(oriNode, oriItem) {
      ...
      // parent 처리
      if (this.partialBatch) {
        this.handleRecursionNodeParents(oriNode.$parent);
      }
      this.$forceUpdate();
    },
    handleRecursionNodeParents(parent, partial = false) {
      if (parent && parent.model) {
        let childNode = parent.$children;
        let selectedChild = childNode.filter((el) => el.model.selected) || [];

        if (childNode.length === selectedChild.length) {
          parent.model.selected = true;
          parent.model.partial = false;
        } else {
          parent.model.selected = false;
          parent.model.partial = selectedChild.length > 0 || partial;
        }

        this.handleRecursionNodeParents(parent.$parent, parent.model.partial);
      }
    },
      ...
      if (this.draggedItem) {
        if (
          ...
          (oriItem[this.childrenFieldName] &&
            oriItem[this.childrenFieldName].findIndex((t) => t.id === this.draggedItem.item.id) !== -1)
        ) {
          return;
        }
    ...
    allSelected() {
      this.initializeData(this.data, { selected: true });
    },
    allUnSelected() {
      this.initializeData(this.data, { selected: false });
    },
    allOpen() {
      this.initializeData(this.data, { opened: true });
    },
    allClose() {
      this.initializeData(this.data, { opened: false });
    }
  },
  created() {
